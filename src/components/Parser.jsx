    ...
    const handleGenerate = () => {
        ...
        for (const raw of lines) {
            ...
            if (!month) {
                ...
                for (const t of datePart.toLowerCase().split(/\s+/)) {
        ...
        createEvents(events, (err, val) => {
            ...
            setMessage(`Generated ${events.length} event(s).`)
        })
    }

    return (<div>
        <div className="flex gap-3">
            <button onClick={handleGenerate} className="px-4 py-2 rounded-md shadow-sm"
                    style={{background: '#16a39a', color: 'white'}}>Generate ICS
            </button>
            {downloadUrl &&
                <a className="px-4 py-2 rounded-md border" href={downloadUrl} download="events.ics">Download .ics</a>}
        </div>
        {message && <div className="mt-3 text-sm text-gray-700">{message}</div>}
        <div className="mt-4 text-xs text-gray-500">Timezone used: <strong>{timezone}</strong></div>
    </div>)
}
